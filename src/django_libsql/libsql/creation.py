...
class DatabaseCreation(SQLiteDatabaseCreation):
    ...
    def create_test_db(self, verbosity=1, autoclobber=False, serialize=True, keepdb=False):
        ...
        if test_database_name and (
            test_database_name.startswith('libsql://') or 
            test_database_name.startswith('wss://') or
            test_database_name.startswith('https://')
        ):
            if verbosity >= 1:
        ...
        if test_database_name and (
            ...
            with self.connection.cursor() as cursor:
                try:
                    ...
                    cursor.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='books_book'")
                    if cursor.fetchone()[0] == 0:
                        # Tables don't exist, run migrations
                        self.log('Tables missing in deserialize_db_from_string, running migrations...')
                        call_command(
                            'migrate',
                            verbosity=0,
                            interactive=False,
                    ...
                    cursor.execute("DELETE FROM gil_test_performance WHERE data LIKE '%Test%'")
                    
                    # Reset auto-increment sequences if they exist
                    cursor.execute("DELETE FROM sqlite_sequence WHERE name IN ('books_book', 'books_review')")
