...
def main():
    ...
    parser.add_argument('--gil-only', choices=['enabled', 'disabled'],
                        help='Run with only one GIL mode')
    ...
    if args.gil_only == 'enabled':
        ...
        print("📊 TEST SUMMARY")
        print(f"{'='*60}")
        print(f"GIL Enabled:  {'✅ PASSED' if results['enabled'] == 0 else '❌ FAILED'}")
        print(f"GIL Disabled: {'✅ PASSED' if results['disabled'] == 0 else '❌ FAILED'}")
