...
class GILComparisonTest(TransactionTestCase):
    ...
    def test_gil_performance_comparison(self):
        ...
        single_result = self.run_threaded_operations(
            num_threads=1, operations_per_thread=10
        )

        # Run multi-threaded test
        multi_result = self.run_threaded_operations(
            num_threads=4, operations_per_thread=10
        )

        # Calculate speedup
        speedup = (
            multi_result["ops_per_sec"] / single_result["ops_per_sec"]
            if single_result["ops_per_sec"] > 0
            else 0
        )
        ...
        print(
            f"  Multi-threaded (4 threads): {multi_result['ops_per_sec']:.2f} ops/sec"
        )
        ...
        if is_gil_disabled():
            ...
            self.assertLess(
                speedup, 2.0, f"Unexpected high speedup {speedup:.2f}x with GIL enabled"
            )
...
"""
        ...
        if "gil_enabled" in results and "gil_disabled" in results:
            ...
            if enabled_speedup and disabled_speedup:
                improvement = (
                    (disabled_speedup - enabled_speedup) / enabled_speedup
                ) * 100
...
class NoGILThreadingTest(TransactionTestCase):
    ...
    def test_high_concurrency_no_gil(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(intensive_operations, i) for i in range(num_threads)
            ]
            results = [f.result() for f in futures]
