...
class ThreadingTestCase(TransactionTestCase):
    ...
    def test_connection_isolation(self):
        ...
        num_threads = 5
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(get_connection_info, i) for i in range(num_threads)]
            results = [f.result() for f in futures]
...
class ThreadingPerformanceTest(TransactionTestCase):
    ...
    def test_performance_single_vs_multi_threaded(self):
        ...
        single_ops_per_sec = (num_operations * 3) / single_duration  # 3 ops per iteration (no delete)
        ...
        multi_ops_per_sec = (num_threads * ops_per_thread * 3) / multi_duration  # 3 ops per iteration
        
        # Report results
        print(f"\nPerformance Comparison (GIL {'DISABLED' if is_gil_disabled() else 'ENABLED'}):")
        print(f"  Single-threaded: {single_ops_per_sec:.2f} ops/sec")
        print(f"  Multi-threaded ({num_threads} threads): {multi_ops_per_sec:.2f} ops/sec")
...
class TursoSpecificTests(TransactionTestCase):
    ...
    def test_turso_sync_with_threading(self):
        ...
        def create_and_verify(thread_id):
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT COUNT(*) FROM testapp_book WHERE title LIKE 'Turso_Sync_%'")
